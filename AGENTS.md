...
- Current non-production decision goal: prepare Manual Review Notes v1 data semantics before any v1 implementation. Manual Review Notes v0 edit means saving a changed human-entered value, clear/delete means confirmed clearing of that saved value, and state/timestamp clarity means showing saved/empty manual note state plus only lead-level update copy such as "Lead last updated" when semantics are truthful. Keep note-specific timestamp schema, note history, reviewer identity, retention/privacy enforcement, production proof, outcome learning, CRM/outreach/analytics/LLM, and manager dashboard v1 separately scoped unless a future approval selects them.
...
- Option A manual review notes are human-entered text only. The explicit local/test-safe API/UI contract is `manualReviewNotes` with derived provenance `human_entered`, backed by the existing `notes` column. Generated suggestion payload fields must be rejected or ignored by persistence paths and must not be stored as manual notes. Saved/empty state labels may use `updatedAt` / `updated_at` only as a lead-level "last updated" signal; do not present it as a note-specific save timestamp.
...
- Production deploy and production DB writes were not performed during PR #25, PR #26, PR #27, or the shipped PR #36-#122 local/test/docs/reviewer-UX/product-planning train.
