...
- Current non-production documentation goal: Manual Review Notes v1 reviewer identity and author attribution semantics are decision-ready only. Manual Review Notes edit means saving a changed human-entered value, clear/delete means confirmed clearing of that saved value, and `manualReviewNotesUpdatedAt` means the last accepted human-entered manual note change/save/clear event. Reviewer identity is not implemented. Lead-level `updatedAt` / `updated_at` remains lead-level only and must not be labeled as a manual-note timestamp. Keep implementation, note history, reviewer identity, retention/privacy enforcement, production proof, outcome learning, CRM/outreach/analytics/LLM, and manager dashboard v1 separately scoped unless a future approval selects them.
...
- Production deploy and production DB writes were not performed during PR #25, PR #26, PR #27, or the shipped PR #36-#123 local/test/docs/reviewer-UX/product-planning train.
