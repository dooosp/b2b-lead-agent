...
- Manual Review Notes v1 Note History / Versioning Decision Packet: `docs/roadmap/manual-review-notes-v1-note-history-versioning-decision-packet.md` records decision-only options for current-value-only behavior, last-write metadata, metadata-only local/test history, full local/test history, audit-grade production history, content retention, event types, clear/delete semantics, UI exposure, and future approval gates. It does not approve implementation, schema/API/runtime/UI changes, D1 migrations, note history, append-only logs, old note value retention, retention/privacy enforcement, generated suggestion history, production action, or production data access.
...
- Current non-production documentation goal: Manual Review Notes v1 note history/versioning semantics are decision-ready only. Manual Review Notes edit means saving a changed human-entered value, clear/delete means confirmed clearing of that saved value, `manualReviewNotesUpdatedAt` means the last accepted human-entered manual note change/save/clear event, and `manualReviewNotesAuthorLabel` remains the fixed non-PII `manual_reviewer` label only. Note history/versioning is not implemented. Old manual note values are not retained in history. Lead-level `updatedAt` / `updated_at` remains lead-level only and must not be labeled as a manual-note timestamp. Keep implementation, note history, retention/privacy enforcement, production proof, outcome learning, CRM/outreach/analytics/LLM, and manager dashboard v1 separately scoped unless a future approval selects them.
...
- Production deploy and production DB writes were not performed during PR #25, PR #26, PR #27, or the shipped PR #36-#126 local/test/docs/reviewer-UX/product-planning train.
