...
- Current non-production implementation goal: Manual Review Notes v1 T1 note-specific timestamp semantics are local/test-safe only. Manual Review Notes edit means saving a changed human-entered value, clear/delete means confirmed clearing of that saved value, and `manualReviewNotesUpdatedAt` means the last accepted human-entered manual note change/save/clear event. Lead-level `updatedAt` / `updated_at` remains lead-level only and must not be labeled as a manual-note timestamp. Keep note history, reviewer identity, retention/privacy enforcement, production proof, outcome learning, CRM/outreach/analytics/LLM, and manager dashboard v1 separately scoped unless a future approval selects them.
