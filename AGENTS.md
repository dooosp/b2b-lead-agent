...
- Current non-production implementation goal: Manual Review Notes v1 H2 metadata-only history is local/test-only. Manual Review Notes edit means saving a changed human-entered value, clear/delete means confirmed clearing of that saved value, `manualReviewNotesUpdatedAt` means the last accepted human-entered manual note change/save/clear event, and `manualReviewNotesAuthorLabel` remains the fixed non-PII `manual_reviewer` label only. Metadata history stores create/edit/clear event metadata only in `manual_review_note_events`; old manual note values are not retained in history. Lead-level `updatedAt` / `updated_at` remains lead-level only and must not be labeled as a manual-note timestamp. Current safe next documentation path is retention/privacy policy decision readiness only. Keep full note history, old note value retention, retention/privacy enforcement, purge/delete jobs, redaction, automated PII detection, export/manager visibility expansion, production proof, outcome learning, CRM/outreach/analytics/LLM, and manager dashboard v1 separately scoped unless a future approval selects them.
...
- Production deploy and production DB writes were not performed during PR #25, PR #26, PR #27, or the shipped PR #36-#128 local/test/docs/reviewer-UX/product-planning train.
