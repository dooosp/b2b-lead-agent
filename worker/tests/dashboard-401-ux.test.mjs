...
test('dashboard page includes branded recovery states for dashboard API auth failures', () => {
  const html = getDashboardPage(createEnv());

  assert.match(html, /리드 파이프라인 현황/);
  assert.match(html, /function renderDashboardState/);
  ...
  assert.match(html, /if\s*\(res\.status === 503\)/);
  assert.match(html, /서버 인증 설정이 누락되었습니다\./);
