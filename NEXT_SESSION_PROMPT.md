...
- Manual Review Notes v1 Note History / Versioning Decision Packet lives at `docs/roadmap/manual-review-notes-v1-note-history-versioning-decision-packet.md`. It is decision-only and covers current-value-only behavior, last-write metadata, metadata-only local/test history, full local/test history, audit-grade production history, content retention, event types, clear/delete semantics, UI exposure, and future approval gates without approving implementation.
...
- Current Manual Review Notes v1 note history/versioning decision-packet approval-intent record: `https://github.com/dooosp/b2b-lead-agent/issues/118#issuecomment-4487570553`. Note history work should remain decision-only unless separately scoped. Do not implement append-only history, old note value retention, generated suggestion history, production rollout, or retention/privacy enforcement from this path.
...
- `docs/roadmap/manual-review-notes-v1-note-history-versioning-decision-packet.md` is decision-only and does not authorize schema/API/runtime/UI behavior, D1 migrations, note history implementation, append-only logs, old note value retention, retention/privacy enforcement, generated suggestion history, production proof, production deploy, production D1, production endpoints, or production logs/secrets.
