...
| --- | --- | --- | --- | --- | --- | --- | --- |
| G1: production D1 migration plan missing | Local/test schema supports current fields/table; no production D1 plan is approved. | Lazy or explicit migration could fail, drift, or write unexpected production schema. | Current production schema inventory, migration plan, dry-run/rehearsal evidence, stop criteria. | DB owner, product owner, ops owner. | Production D1 migration plan packet. | Production D1 migration/access/write, deploy that depends on migration. | Keep production D1 HOLD; prepare M1 plan only. |
| G2: production rollback/backout plan missing | No approved rollback path for manual notes production use. | Failed deploy or migration could leave partial schema/data or broken reviewer flows. | Rollback owner, previous artifact, deploy reversal plan, D1 backout rules, no-data-loss stop criteria. | Ops owner plus DB owner. | Rollback/backout packet. | Production proof execution, deploy, migration. | Require rollback packet before P4/P5. |
...
- stop on stale SHA, missing owner, failing CI, missing rollback, unclear D1
  migration path, missing privacy/legal gate, unsafe evidence, or unapproved
