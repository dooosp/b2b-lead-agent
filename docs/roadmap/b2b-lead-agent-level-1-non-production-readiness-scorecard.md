...
| Post-approval decision simulator | `PASS_LOCAL`, `HOLD_PRODUCTION` | `scripts/level1-post-approval-decision-simulator.mjs`; `worker/tests/level1-post-approval-decision-simulator.test.mjs`; `docs/roadmap/b2b-lead-agent-level-1-post-approval-decision-simulator-synthetic-packets-non-production.json`; `tmp/codex/level1-post-approval-decision-simulator-non-production.json`; `docs/roadmap/b2b-lead-agent-level-1-post-approval-decision-simulator-non-production.md`; `npm run proof:level1:post-approval-simulator` | Local-only simulator consumes checked-in synthetic Issue #165 packets only and returns `HOLD`, `BLOCKED`, or `READY_FOR_SEPARATE_HUMAN_EXECUTION`. Complete synthetic packets can reach only the separate-human-execution decision; unsafe, stale, vague, broad endpoint, D1-private, secret/raw-auth, destructive SQL, customer-policy-gap, rollback-gap, contradictory, or `productionReady:true` inputs fail closed. It emits redacted `NOT_PRODUCTION_EVIDENCE`, keeps `productionReady:false`, `productionReviewerWorkflowReady:false`, and `proofExecutionApproved:false`, and does not execute proof. |
| Level 1 package/CI regression gate | `PASS_LOCAL`, `HOLD_PRODUCTION` | `package.json`; `.github/workflows/ci.yml`; `worker/tests/workflow-contract.test.mjs`; `npm run check:level1` | Durable local-only package gate runs Level 1 auth adapter/scaffold, route/privacy, proof-preflight, approval dry-run, change-control manifest dry-run, operator rehearsal, closure dashboard, approval intake, post-approval simulator, release-evidence redaction, artifact redaction, and generated-suggestion/manual-note boundary coverage. CI runs it without secrets, deploy, Wrangler, D1 bindings, endpoint calls, or production inputs. |
...
- post-approval decision simulator tests and local JSON/Markdown artifact writer.
