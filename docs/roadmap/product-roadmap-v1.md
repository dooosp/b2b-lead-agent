...
Post-PR119, Manager / Reviewer Summary v0 is shipped from existing
...
The safest current product step is completing and validating that scoped
human-entered manual note path without expanding into edit/delete, note history,
...
| Local/test implementation | Option A manual review notes | Issue #118 approval record after PR #119 | Saves only human-entered `manualReviewNotes` through explicit manual note controls and `PATCH /api/leads/:id`, backed by the existing `notes` row value with derived `human_entered` provenance. Generated reviewer note suggestions remain copy-only response helper text, direct generated-suggestion patch fields are rejected, and no generated suggestion snapshot, production D1, production endpoint, CRM/outreach, analytics, LLM, edit/delete, note history, or manager dashboard v1 scope is added. |
