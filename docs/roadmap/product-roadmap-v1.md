...
Post-PR126, Manager / Reviewer Summary v0 is shipped from existing
...
PRs #122 through #126 shipped reviewer-safe saved/empty state clarity,
...
decision packet and fixed generic author label. The safest current product step
is keeping note history/versioning decision-ready only without implementing
append-only history, old note value retention, authenticated identity,
display-name fields, author audit trails, generated suggestion history,
generated suggestion attribution, generated suggestion persistence,
retention/privacy enforcement, outcome learning, summary v1, or production
observation.
...
At this source-of-truth sync preflight, PR #119 through PR #126 are merged, Issue #118 is closed as completed for the plan-only record and local/test-safe Option A implementation, and the current approval record authorizes only a docs-only Manual Review Notes v1 note history/versioning decision packet. Issue #100 is closed as completed for its recorded local/test-safe UX findings. Issue #111 is closed as completed for Manager / Reviewer Summary v0 UX intake. Issue #113 is closed as completed for Option E. Issue #115 is closed as completed for copy-only reviewer note suggestion UX findings. Issue #34 is closed as completed after GitHub-only closeout, with no further production execution approved. New work should start from current `master`, not from old stacked branches. The active safe path is decision-only note history/versioning documentation; production proof, production saved-note use, manager dashboard expansion, outcome learning, production schema migration, note history implementation, append-only logs, old note value retention, authenticated reviewer identity, real reviewer display names/emails, author audit trails, retention/privacy enforcement, generated suggestion history, generated suggestion attribution, generated suggestion persistence, and broader persistence work should not begin unless separately selected and scoped.
...
| Local/test implementation | Option A manual review notes | Issue #118 approval records after PR #119 through PR #126 | Saves only human-entered `manualReviewNotes` through explicit manual note controls and `PATCH /api/leads/:id`, backed by the existing `notes` row value with derived `human_entered` provenance while saved text exists. Manual Review Notes v0 edit means saving a changed human-entered value, and clear/delete means confirmed clearing of that saved value. PR #122 makes saved vs empty state explicit and may show only lead-level `updatedAt` / `updated_at` as lead last-update copy when no note-specific timestamp exists. PR #124 adds `manualReviewNotesUpdatedAt` / `manual_review_notes_updated_at` only for accepted human-entered manual note create/edit/clear events. PR #126 adds only `manualReviewNotesAuthorLabel: "manual_reviewer"` / `manual_review_notes_author_label` on accepted human-entered manual note create/edit/clear events. Generated reviewer note suggestions remain copy-only response helper text, direct generated-suggestion patch fields are rejected, and no generated suggestion snapshot, generated suggestion attribution, generated suggestion history, production D1, production endpoint, CRM/outreach, analytics, LLM, note history, old note value retention, authenticated reviewer identity, retention/privacy enforcement, or manager dashboard v1 scope is added. |
| Planning plus local/test implementation | Manual Review Notes v1 data semantics | `docs/roadmap/manual-review-notes-v1-data-semantics-decision-packet.md` plus Issue #118 approval records | Prepared decisions for note-specific timestamp, reviewer identity, note history/versioning, retention/privacy, and production-readiness gates. T1 note-specific timestamp was later implemented locally by PR #124, and the fixed generic author label was later approved for local/test-only implementation. No real reviewer identity, note history, retention/privacy enforcement, production proof, production endpoint call, production logs/secrets access, generated suggestion persistence, or generated suggestion snapshot is approved. |
| Planning plus local/test implementation | Manual Review Notes v1 reviewer identity / author attribution | `docs/roadmap/manual-review-notes-v1-reviewer-identity-decision-packet.md` plus Issue #118 approval records | Prepares decisions for reviewer identity source, author display semantics, author update rules, privacy/PII handling, blocked areas, future approval blocks, and production-readiness gates. The later approval record allows only the fixed local/test `manual_reviewer` label and UI copy `수동 리뷰어`. No authenticated identity, display-name fields, email, author audit trails, note history, retention/privacy enforcement, generated suggestion attribution, generated suggestion persistence, production proof, production endpoint call, or production logs/secrets access is approved. |
| Planning | Manual Review Notes v1 note history / versioning | `docs/roadmap/manual-review-notes-v1-note-history-versioning-decision-packet.md` plus Issue #118 approval record | Prepares decisions for current-value-only behavior, last-write metadata, metadata-only local/test history, full local/test history, audit-grade production history, history content, event types, clear/delete semantics, UI exposure, retention/privacy, and future approval gates. Recommends H1 as the current effective local/test state: current value plus last-write metadata only, no old note text retention, no append-only history yet, generated suggestions excluded from history, and production HOLD. No note history implementation, append-only log, old note value retention, generated suggestion history, retention/privacy enforcement, production proof, production endpoint call, or production logs/secrets access is approved. |
