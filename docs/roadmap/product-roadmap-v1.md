...
- `docs/roadmap/next-product-track-decision-packet.md` is the post-PR107 planning packet that selected manager/reviewer summary v0 as the safest local/test-safe product slice. After PR #114, Issue #113, PR #119, and the Issue #118 implementation approval records, use it as historical decision context; `docs/roadmap/saved-review-notes-decision-packet.md` is the planning artifact plus Option E record for generated suggestions, and `docs/roadmap/manual-review-notes-v1-data-semantics-decision-packet.md` is the decision-only packet for v1 manual note data semantics. Generated reviewer note suggestions are copy-only helper text, not saved, not sent, and not human-authored saved notes. Option A local/test-safe manual notes persist only human-entered `manualReviewNotes` with derived `human_entered` provenance. Edit/delete v0 is limited to edit-by-resave and confirmed clear-by-empty-value. Saved/empty state clarity may use lead-level `updatedAt` / `updated_at` only as lead last-update copy, not as a manual-note-specific save timestamp. Note-specific timestamps, note history, reviewer identity, retention/privacy enforcement, outcome learning, production observation, and any summary v1 dashboard expansion remain separate decisions.
...
Post-PR122, Manager / Reviewer Summary v0 is shipped from existing
...
PR #122 shipped reviewer-safe saved/empty state clarity with truthful
lead-level update-state labeling. The safest current product step is making
Manual Review Notes v1 data semantics decision-ready without implementing
note-specific timestamps, note history, reviewer identity, generated suggestion
persistence, retention/privacy enforcement, outcome learning, summary v1, or
...
At this source-of-truth sync preflight, PR #119 through PR #122 are merged, Issue #118 is closed as completed for the plan-only record and local/test-safe Option A implementation, and the current approval-intent record authorizes only a docs-only Manual Review Notes v1 data semantics decision packet. Issue #100 is closed as completed for its recorded local/test-safe UX findings. Issue #111 is closed as completed for Manager / Reviewer Summary v0 UX intake. Issue #113 is closed as completed for Option E. Issue #115 is closed as completed for copy-only reviewer note suggestion UX findings. Issue #34 is closed as completed after GitHub-only closeout, with no further production execution approved. New work should start from current `master`, not from old stacked branches. The active safe path is decision-only Manual Review Notes v1 data semantics; production proof, production saved-note use, manager dashboard expansion, outcome learning, schema migration, note-specific timestamp schema, note history, reviewer identity, retention/privacy enforcement, generated suggestion persistence, and broader persistence work should not begin unless separately selected and scoped.
...
| Local/test implementation | Option A manual review notes | Issue #118 approval record after PR #119, PR #120, PR #121, and PR #122 | Saves only human-entered `manualReviewNotes` through explicit manual note controls and `PATCH /api/leads/:id`, backed by the existing `notes` row value with derived `human_entered` provenance while saved text exists. Manual Review Notes v0 edit means saving a changed human-entered value, and clear/delete means confirmed clearing of that saved value. PR #122 makes saved vs empty state explicit and may show only lead-level `updatedAt` / `updated_at` as lead last-update copy, never as manual-note-specific saved time. Generated reviewer note suggestions remain copy-only response helper text, direct generated-suggestion patch fields are rejected, and no generated suggestion snapshot, production D1, production endpoint, CRM/outreach, analytics, LLM, note history, reviewer identity, retention/privacy enforcement, or manager dashboard v1 scope is added. |
| Planning | Manual Review Notes v1 data semantics | `docs/roadmap/manual-review-notes-v1-data-semantics-decision-packet.md` plus Issue #118 approval-intent record | Prepares decisions for note-specific timestamp, reviewer identity, note history/versioning, retention/privacy, and production-readiness gates only. No implementation, schema/API/runtime behavior, D1 migration, production proof, production endpoint call, production logs/secrets access, generated suggestion persistence, or generated suggestion snapshot is approved. |
