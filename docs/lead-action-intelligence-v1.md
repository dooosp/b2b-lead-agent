...
- Opportunity Workbench renders the full Lead Action Intelligence panel with action, reason, risk flags, missing-info prompts, stakeholder angle, follow-up draft, and generated copy-only reviewer note suggestions. Under Issue #113 Option E, generated suggestions are not saved, sent, or treated as human-authored saved notes. The lead-detail Workbench also exposes copy-friendly note controls, manual-copy fallback, non-mutating shortcuts, shortcut help, and browser-memory activity feedback for the current page session.
