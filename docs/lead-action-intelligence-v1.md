...
- No production D1 migration or production schema proof. The local/test schema includes only the current-value `manual_review_notes_author_label` column for the fixed generic label.
...
- `worker/tests/manual-review-notes.test.mjs` covers human-entered manual note create/edit/clear setting `manualReviewNotesAuthorLabel: "manual_reviewer"`, unchanged saves not inventing labels, generated suggestion non-attribution, and generated batch inserts not creating saved manual notes or author labels.
- `worker/tests/d1-schema-contract.test.mjs` covers the local/test D1 `manual_review_notes_author_label` column.
