...
- No production D1 migration or production schema proof. The local/test schema includes current-value `manual_review_notes_author_label` plus `manual_review_note_events` metadata-only history for create/edit/clear event type, timestamp, lead id, and fixed generic author label.
- No old manual note value retention, generated suggestion history, full note text history, audit-grade history, retention job, purge job, or privacy enforcement workflow.
