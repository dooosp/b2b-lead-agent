...
- Option A manual review notes behavior on `/leads`: a human-entered manual note saves, edits, and clears through the explicit `manualReviewNotes` API field, reads back from fake-D1 as `manualReviewNotes`, exposes `manualReviewNotesProvenance: "human_entered"` only while saved note text exists, and does not persist generated reviewer note suggestions onto lead records
