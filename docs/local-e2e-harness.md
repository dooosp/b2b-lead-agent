...
- Option A manual review notes behavior on `/leads`: a human-entered manual note saves, edits, and clears through the explicit `manualReviewNotes` API field, reads back from fake-D1 as `manualReviewNotes`, exposes `manualReviewNotesProvenance: "human_entered"` only while saved note text exists, updates `manualReviewNotesUpdatedAt` only for accepted human-entered manual note change/save/clear events, renders note-specific timestamp copy when present, and does not persist generated reviewer note suggestions onto lead records
