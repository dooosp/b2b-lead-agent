...
import { loadAndAggregateHumanValidationSessionFiles } from './lib/pursuit-workbench-human-validation-files.mjs';
...
function safeFailure(error) {
  return {
    ...
    decision: 'INCOMPLETE',
    recordIds: [],
    failureCodes: [typeof error?.code === 'string' ? error.code : 'SESSION_VALIDATION_FAILED']
  };
}
...
try {
  ...
  process.stdout.write(`${JSON.stringify(loadAndAggregateHumanValidationSessionFiles(), null, 2)}\n`);
} catch (error) {
